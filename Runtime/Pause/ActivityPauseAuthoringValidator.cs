...
namespace Immersive.Framework.Pause
{
    ...
    public static class ActivityPauseAuthoringValidator
    {
        public static PauseActivityBindingIntentResolution ResolveDeclarations(
            IEnumerable<ActivityPauseAuthoring> declarations,
            string source)
        {
            string normalizedSource = source.NormalizeTextOrFallback(
                nameof(ActivityPauseAuthoringValidator));
            var collected = new List<ActivityPauseAuthoring>();

            if (declarations != null)
            {
                foreach (ActivityPauseAuthoring declaration in declarations)
                {
                    if (declaration == null)
                    {
            ...
            string source)
        {
            ...
            for (int rootIndex = 0; rootIndex < roots.Count; rootIndex++)
            {
                ...
                ActivityPauseAuthoring[] declarations =
                    root.GetComponentsInChildren<ActivityPauseAuthoring>(true);
                for (int declarationIndex = 0;
                    declarationIndex < declarations.Length;
                    declarationIndex++)
                {
                    ActivityPauseAuthoring declaration = declarations[declarationIndex];
        ...
        private static PauseActivityBindingIntentResolution ResolveCollected(
            IReadOnlyList<ActivityPauseAuthoring> declarations,
            string source)
        {
            ...
            ActivityPauseAuthoring declaration = declarations[0];
            if (declaration == null)
            {
                return PauseActivityBindingIntentResolution.Invalid(
                    count,
