...
namespace Immersive.Framework.CycleReset
{
    ...
    {
        internal static ActivityCycleResetTriggerBinderResult TryBind(IReadOnlyList<GameObject> roots, IActivityCycleResetRuntimePort activityCycleResetRuntime)
        {
            int rootCount=CountUniqueRoots(roots);
            if(activityCycleResetRuntime==null) return ActivityCycleResetTriggerBinderResult.Rejected("RejectedMissingActivityCycleResetRuntime",$"Activity Cycle Reset trigger binding requires an Activity Cycle Reset runtime port. roots='{rootCount}' triggers='0' bound='0' idempotent='0' rejected='0'.",rootCount,0,0,0,0);
            List<ActivityCycleResetTrigger> triggers=CollectTriggers(roots);
            if(triggers.Count==0) return ActivityCycleResetTriggerBinderResult.OptionalAbsent(rootCount);
            ...
            return rejected>0 ? ActivityCycleResetTriggerBinderResult.Rejected("RejectedTriggerBinding",$"Activity Cycle Reset trigger binding failed. roots='{rootCount}' triggers='{triggers.Count}' bound='{bound}' idempotent='{idempotent}' rejected='{rejected}'. {string.Join(" ",issues)}",rootCount,triggers.Count,bound,idempotent,rejected) : ActivityCycleResetTriggerBinderResult.Completed(rootCount,triggers.Count,bound,idempotent);
        }
