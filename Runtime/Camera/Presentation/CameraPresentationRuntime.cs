...
namespace Immersive.Framework.Camera
{
    ...
    internal sealed class CameraPresentationRuntime : IDisposable
    {
        ...
        internal void DisposeTerminal()
        {
            ...
            if (_membership != null)
            {
                CameraCompositionMembershipResult cleared =
                    _membership.Clear();
                _currentMembership = cleared.Snapshot;
            }

            // Application termination is not a normal lifecycle transition.
            // Unity may already have invalidated scene-backed Camera objects, so
            // terminal cleanup must not attempt request arbitration or Rig apply.
            _membership = null;
            _currentMembership = null;
            _requestPublisher = null;
            _availability = null;
        ...
        private bool TryResolveRequestLifecycle(
            out CameraRequestOwnerKind ownerKind,
            out CameraRequestLifetimeKind lifetimeKind,
            out string scopeId,
            out string diagnostic)
        {
            ownerKind = CameraRequestOwnerKind.Undefined;
            lifetimeKind = CameraRequestLifetimeKind.Undefined;
            scopeId = string.Empty;

            if (!_lifecycleContext.IsValid)
            {
                diagnostic =
                    "Camera Presentation request publication requires explicit Session, Route or Activity lifecycle ownership.";
                return false;
            }
            ...
            switch (_lifecycleContext.Scope)
            {
                ...
                default:
                    diagnostic =
                        $"Camera Presentation lifecycle scope '{_lifecycleContext.Scope}' is not supported. Expected Session, Route or Activity.";
                    return false;
            }
        }
                ...
                CameraRequestOwnerKind ownerKind)
        {
            switch (ownerKind)
            {
                ...
                default:
                    throw new InvalidOperationException(
                        $"Camera Presentation request owner '{ownerKind}' is not supported.");
            }
        }
