...
namespace Immersive.Framework.Editor.PlayerParticipation
{
    ...
    {
        public SceneLocalPlayerAdmissionAuthoringResult(bool succeeded, SceneLocalPlayerAdmissionAuthoringStatus status, string message, bool evidenceCreated, bool evidenceUpdated)
        {
            Succeeded = succeeded;
            Status = status;
    ...
    {
        public static SceneLocalPlayerAdmissionAuthoringResult Validate(SceneLocalPlayerAdmissionAuthoring authoring, bool logDiagnostics = true)
        {
            SceneLocalPlayerAdmissionAuthoringResult result = ValidateCore(authoring, true);
            Record(authoring, result, logDiagnostics);
            return result;
        }

        public static SceneLocalPlayerAdmissionAuthoringResult ApplyOrRebuild(SceneLocalPlayerAdmissionAuthoring authoring, bool logDiagnostics = true, bool useUndo = true)
        {
            SceneLocalPlayerAdmissionAuthoringResult preflight = ValidateMaterializationInputs(authoring);
            ...
            SceneLocalPlayerAdmissionAuthoringResult compositionPreflight =
            ...
            SceneLocalPlayerAdmissionAuthoringResult resolved = ResolveOrMaterialize(authoring, useUndo, out PlayerActorRuntimeHost runtimeHost, out GameObject presentation, out bool createdRuntimeHost, out bool createdPresentation);
            ...
            if (useUndo) Undo.RecordObject(authoring, "Apply Scene-Provided Player Evidence");
            ...
            SceneLocalPlayerAdmissionAuthoringResult validation = ValidateCore(authoring, true);
            SceneLocalPlayerAdmissionAuthoringResult final = validation.Succeeded
                ? new SceneLocalPlayerAdmissionAuthoringResult(true, SceneLocalPlayerAdmissionAuthoringStatus.Valid, createdRuntimeHost || createdPresentation ? "Scene-Provided Player Runtime Host and selected Presentation were materialized under the exact Actor Mount and Presentation Mount." : "Scene-Provided Player Runtime Host and Presentation were preserved and typed Presentation evidence was refreshed.", evidenceCreated, evidenceUpdated)
                : validation;
            Record(authoring, final, logDiagnostics);
            return final;
        }

        private static SceneLocalPlayerAdmissionAuthoringResult ValidateCore(SceneLocalPlayerAdmissionAuthoring authoring, bool requireEvidence)
        {
            SceneLocalPlayerAdmissionAuthoringResult inputs = ValidateMaterializationInputs(authoring);
            ...
            {
                return Failure(SceneLocalPlayerAdmissionAuthoringStatus.MissingProfileEvidence, "Scene Player Runtime Host or Presentation is missing. Run Apply / Rebuild to materialize the selected composition.");
            }
            if (!ValidateSceneComposition(authoring, out string issue))
            {
                return Failure(SceneLocalPlayerAdmissionAuthoringStatus.IncompatibleProfileEvidence, issue);
            }
            if (!requireEvidence)
            {
                return new SceneLocalPlayerAdmissionAuthoringResult(true, SceneLocalPlayerAdmissionAuthoringStatus.Valid, "Scene-Provided Player Runtime Host and Presentation composition is valid.", false, false);
            }
            if (!authoring.TryValidateRuntimeEvidence(out issue))
            {
                return Failure(authoring.HasTypedActorEvidence ? SceneLocalPlayerAdmissionAuthoringStatus.IncompatibleProfileEvidence : SceneLocalPlayerAdmissionAuthoringStatus.MissingProfileEvidence, issue);
            }
            ...
            {
                return Failure(SceneLocalPlayerAdmissionAuthoringStatus.MissingProfileEvidence, "Scene Presentation lacks compatible serialized Presentation evidence. Run Apply / Rebuild.");
            }
            return new SceneLocalPlayerAdmissionAuthoringResult(true, SceneLocalPlayerAdmissionAuthoringStatus.Valid, "Scene-Provided Player authoring and Presentation evidence are valid.", false, false);
        }

        private static SceneLocalPlayerAdmissionAuthoringResult ValidateMaterializationInputs(SceneLocalPlayerAdmissionAuthoring authoring)
        {
            ...
            if (!PrefabUtility.IsPartOfPrefabAsset(profile.PresentationPrefab)) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.InvalidActorProfile, $"Actor Profile Presentation '{profile.PresentationPrefab.name}' must be a prefab asset.");
            if (profile.PresentationPrefab.GetComponentInChildren<PlayerInput>(true) != null || profile.PresentationPrefab.GetComponentInChildren<ActorDeclaration>(true) != null || profile.PresentationPrefab.GetComponentInChildren<PlayerActorRuntimeHost>(true) != null) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.InvalidActorProfile, "Actor Profile Presentation prefab must not contain PlayerInput, Actor declarations or Player Actor Runtime Host infrastructure.");
            ...
            if (host.PlayerInput.gameObject != host.gameObject || host.GetComponentsInChildren<PlayerInput>(true).Length != 1 || host.ActorMount.GetComponentInChildren<PlayerInput>(true) != null) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.InvalidHost, "Local Player Host requires exactly one PlayerInput outside Actor Mount.");
            return new SceneLocalPlayerAdmissionAuthoringResult(true, SceneLocalPlayerAdmissionAuthoringStatus.Valid, "Scene-Provided Player materialization inputs are valid.", false, false);
        }

        private static SceneLocalPlayerAdmissionAuthoringResult ResolveOrMaterialize(SceneLocalPlayerAdmissionAuthoring authoring, bool useUndo, out PlayerActorRuntimeHost runtimeHost, out GameObject presentation, out bool createdRuntimeHost, out bool createdPresentation)
        {
            ...
            if (runtimeHost == null)
            {
                ...
                if (existing.Length > 1) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.InvalidHost, "Actor Mount contains multiple Player Actor Runtime Hosts. Apply / Rebuild will not guess ownership.");
                if (existing.Length == 1) runtimeHost = existing[0];
                else
                {
                    if (EditorUtility.IsPersistent(authoring.gameObject)) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.MissingProfileEvidence, "Runtime Host materialization cannot modify a prefab asset directly from the Project view.");
                    ...
                    runtimeHost = runtimeHostRoot != null
                        ...
                        : null;
                    if (runtimeHost == null) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.MissingProfileEvidence, "Apply / Rebuild could not materialize the configured Player Actor Runtime Host prefab.");
                    if (useUndo) Undo.RegisterCreatedObjectUndo(runtimeHost.gameObject, "Create Scene-Provided Player Actor Runtime Host");
                    createdRuntimeHost = true;
                }
            }

            if (!ValidateRuntimeHostInstance(authoring, runtimeHost, out string runtimeHostIssue)) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.InvalidHost, runtimeHostIssue);
            Transform presentationMount = runtimeHost.PresentationMount;
            if (presentation == null)
            {
                if (presentationMount.childCount > 1) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.InvalidHost, "Presentation Mount contains multiple children. Apply / Rebuild will not guess the selected Presentation.");
                ...
                else
                {
                    ...
                    if (presentation == null) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.MissingProfileEvidence, "Apply / Rebuild could not materialize the selected Presentation prefab.");
                    if (useUndo) Undo.RegisterCreatedObjectUndo(presentation, "Create Scene-Provided Actor Presentation");
                    createdPresentation = true;
                }
            }

            if (!ValidatePresentationInstance(authoring, runtimeHost, presentation, out string presentationIssue)) return Failure(SceneLocalPlayerAdmissionAuthoringStatus.IncompatibleProfileEvidence, presentationIssue);
            return new SceneLocalPlayerAdmissionAuthoringResult(true, SceneLocalPlayerAdmissionAuthoringStatus.Valid, "Scene Runtime Host and Presentation composition resolved.", false, false);
        }

        private static SceneLocalPlayerAdmissionAuthoringResult PreflightComposition(
        ...
        private static bool ValidateSceneComposition(SceneLocalPlayerAdmissionAuthoring authoring, out string issue)
        {
        ...
        private static bool ValidateRuntimeHostInstance(SceneLocalPlayerAdmissionAuthoring authoring, PlayerActorRuntimeHost runtimeHost, out string issue)
        {
            issue = string.Empty;
            ...
            {
                issue = "Scene Player Actor Runtime Host does not match the exact Runtime Host prefab provided by the Local Player Host.";
                return false;
            }
            return true;
        }

        private static bool ValidatePresentationInstance(SceneLocalPlayerAdmissionAuthoring authoring, PlayerActorRuntimeHost runtimeHost, GameObject presentation, out string issue)
        {
            issue = string.Empty;
            if (runtimeHost == null || presentation == null || presentation.transform.parent != runtimeHost.PresentationMount) { issue = "Scene Presentation must be a direct child of the exact Player Actor Runtime Host Presentation Mount."; return false; }
            if (presentation.GetComponentInChildren<PlayerInput>(true) != null || presentation.GetComponentInChildren<ActorDeclaration>(true) != null || presentation.GetComponentInChildren<PlayerActorRuntimeHost>(true) != null) { issue = "Scene Presentation must not contain PlayerInput, Actor declarations or Player Actor Runtime Host infrastructure."; return false; }
            if (!AreSamePrefabAsset(ResolveSourcePrefab(presentation), authoring.ActorProfile.PresentationPrefab)) { issue = "Scene Presentation prefab source does not match the selected Actor Profile Presentation prefab. Apply / Rebuild will not silently replace conflicting authored content."; return false; }
            return true;
        }
        ...
        private static bool EnsurePresentationEvidence(GameObject presentation, ActorProfile profile, bool useUndo, out bool updated)
        {
            ...
            if (updated)
            {
                if (useUndo) Undo.RecordObject(evidence, "Apply Scene Player Presentation Evidence");
        ...
        private static SceneLocalPlayerAdmissionAuthoringResult Failure(SceneLocalPlayerAdmissionAuthoringStatus status, string message) => new SceneLocalPlayerAdmissionAuthoringResult(false, status, message, false, false);

        private static void Record(SceneLocalPlayerAdmissionAuthoring authoring, SceneLocalPlayerAdmissionAuthoringResult result, bool logDiagnostics)
        {
            ...
            FrameworkLogger.Create(typeof(SceneLocalPlayerAdmissionAuthoringUtility)).Info($"[Immersive.Framework][SceneProvidedPlayer] status='{result.Status}' succeeded='{result.Succeeded}' createdEvidence='{result.EvidenceCreated}' updatedEvidence='{result.EvidenceUpdated}' diagnostic='{result.Message}'.");
        }
    }
}
