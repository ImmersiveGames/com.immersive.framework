...
`FindObjectsByType` is not a provisioning bootstrap path. Provisioning is
received through the explicit `UIGlobal` host registration; no loaded-scene
discovery compatibility path remains.
...
PausePlayerInputBinding
  ...
  -> InputModeRuntimeContext transaction
...
3. Run the focused Pause product binding, request and action-map posture cases.
