...
`PlayerActorDeclaration` adds only Player-specific contracts and bridges, such as:

```text
typed access to Unity PlayerInput
runtime PlayerSlot assignment/configuration
PlayerInputManager join/materialization handoff
...
For the current local Player product model, `PlayerActorDeclaration` requires Unity's `PlayerInput` component.

Representative implementation constraint:
...
`RequireComponent` improves authoring but does not replace explicit prefab and runtime validation.

`PlayerInput` alone does not declare a Player Actor. The specialized declaration is `PlayerActorDeclaration`; `PlayerInput` is its required local-input dependency.
...
The executable P3 minimum is temporarily:
...
-> validates PlayerActorDeclaration and PlayerInput
-> binds PlayerSlotProfile and selected ActorProfile
-> assigns runtime Actor identity
...
-> RuntimeContent materializes MageActorHost
-> finds PlayerActorDeclaration inherited as ActorDeclaration
-> validates required Unity PlayerInput
...
-> Logical Actor Host releases through RuntimeContent
...
Those decisions must preserve the three-layer separation established here.
...
A local Player Actor host contains PlayerActorDeclaration inheriting ActorDeclaration,
and does not contain a second ActorDeclaration component.

PlayerActorDeclaration requires Unity PlayerInput for the current local Player model.
...
```text
Docs: freeze Actor and Player declaration inheritance model
```
