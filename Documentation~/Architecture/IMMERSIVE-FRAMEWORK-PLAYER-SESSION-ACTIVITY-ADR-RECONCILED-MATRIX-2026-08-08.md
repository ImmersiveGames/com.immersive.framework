...
| C07 | Consumer boundary | TECHNICAL REVIEW | Scoped command/observation reachability follows the boundary already proposed by IF-ADR-015: typed, explicitly scoped, lifetime-explicit and cross-scene-capable, with no service locator, scene search, reflection or second Player authority. The next phase must determine how the current runtime actually realizes this boundary. | **ALIGNED WITH IF-ADR-015 — PROPOSED** | IF-ADR-015 (Proposed), constrained by IF-ADR-001 | Keep as aligned with ADR-015, but do not treat as accepted normative contract until ADR-015 is accepted/updated. Architecture boundary is substantially specified; implementation reachability is the next-phase question. |
...
- No QA plan yet.
- No FIRSTGAME migration plan yet.
