...
finds PlayerActorDeclaration
validates required PlayerInput
binds PlayerSlotProfile
binds selected ActorProfile only when one is already resolved
assigns ActorId and contextual runtime state
...
12. Framework validates PlayerActorDeclaration and PlayerInput.
13. Framework binds PlayerSlotProfile and ActorId.
14. Framework binds ActorProfile only when already explicitly resolved.
...
The current executable P3 minimum remains:
...
The final product minimum also requires:
...
`PlayerInputManager` creates the configured local Player host. The framework then validates and completes its product identity and contextual composition.

The exact boundary between:

```text
fixed Local Player Host content
ActorProfile-specific logical content
Presentation/Skin content
```

is intentionally out of scope and is the next architecture decision.
...
local Player host
  physical provisioning through PlayerInputManager
  framework admission and contextual ownership afterward
```

A local Player may still receive RuntimeContent-managed child content, composition or Presentation after provisioning, subject to later decisions.
...
-> request Slot 1 join
-> PlayerInputManager provisions one local Player
-> framework binds the configured/default ActorProfile
...
the final fixed Local Player Host composition
how ActorProfile-specific logical content is applied after provisioning
...
PlayerActorDeclaration and PlayerInput are validated on the created host.
...
The configured manager prefab and ActorProfile-specific composition need a
...
## DG-P3-03 resolution — Minimum Local Player Join contract

Status: Accepted in P3G.1  
Date: 2026-07-13

The minimum ordinary local join request is intentionally smaller than the complete provisioning operation.
...
The ordinary request does not contain:

```text
PlayerSlotId or PlayerSlotProfile
playerIndex
splitScreenIndex
ActorProfile
ActorId
Route or Activity identity
Session context id supplied by the caller
```
...
```text
Slot identity
  is resolved atomically by PlayerParticipationRuntimeContext using
  First Available By Configured Order.

Session authority
  is established by the injected host-scoped provisioning bridge and its
  PlayerParticipationRuntimeContext reference, not by caller-authored evidence.

playerIndex / splitScreenIndex
  are Unity technical outputs or advanced provisioning controls and are not
  required by the first product path.

Actor selection
  belongs to P3H and must not be hidden inside local provisioning.
```

`PairWithDevice` and `ControlScheme` are optional technical hints. They are passed to the Unity Input System only when explicitly present. Their absence means the bridge uses the manager's ordinary manual provisioning policy; it does not invent a fallback device or control scheme.

The bridge creates an internal typed operation identity before invoking Unity provisioning:
...
PendingLocalPlayerJoin
  ...
  Request
  Reserved Slot token
  Expected PlayerInputManager
  State
  Callback evidence
  Direct JoinPlayer return evidence
```

The Pending Join must exist after Slot reservation and before `PlayerInputManager.JoinPlayer(...)` is called.

The direct non-null `PlayerInput` returned by `JoinPlayer(...)` is the primary correlation evidence. The manager's joined callback is confirmation and divergence detection. Either may occur first during the synchronous call, so the bridge records both and requires them to resolve to the same `PlayerInput` instance.
...
LocalPlayerJoinResult
  ...
  OperationId
  Request
  Reservation result/token
  Reserved/Joined Slot snapshot
  PlayerInput evidence
  PlayerActorDeclaration evidence
  Unity playerIndex diagnostic evidence
  Callback confirmation state
  Reservation rollback result when applicable
  Message
```
...
Status names may be mechanically adjusted during implementation to match package naming conventions, but these semantic outcomes are frozen.

### Correlation rules

```text
1. Validate request and injected Session/manager references.
2. Reserve the next eligible Slot through PlayerParticipationRuntimeContext.
3. Create PendingLocalPlayerJoin using the returned reservation token.
4. Publish/store the pending operation before JoinPlayer is called.
5. Invoke PlayerInputManager.JoinPlayer through one typed bridge.
6. Record the direct returned PlayerInput or explicit null failure.
7. Record joined callback evidence when received.
8. Reject if direct and callback evidence refer to different instances.
9. Validate PlayerInput and PlayerActorDeclaration on the returned root.
10. Commit the reservation with TryMarkJoined.
11. On any pre-commit failure, call TryReleaseReservation exactly once.
12. Return both the original failure and rollback evidence.
```

No arbitrary frame delay, hierarchy/name lookup or sibling `OnEnable` ordering is allowed.

### PlayerInputManager authoring boundary

The provisioning bridge receives an explicit `PlayerInputManager` reference from authoring/injection. It must validate at minimum:

```text
manager reference exists
manual join behavior is configured
player prefab exists
configured technical player ceiling does not contradict authored Slots
manager belongs to the intended Session composition
```

The bridge must not use `PlayerInputManager.instance` as its operating dependency.

### Actor and control boundary

P3G validates that the provisioned root contains the required `PlayerInput` and `PlayerActorDeclaration` evidence. It does not:

```text
select ActorProfile
apply Actor-specific logical composition
create occupancy
activate gameplay input
activate Player camera participation
materialize Presentation/Skin
```

Existing InputMode / PlayerInput adapters remain downstream consumers after Session join and contextual admission. They are not called as an implicit side effect of the provisioning request.

### Rollback policy

Reservation rollback is mandatory for every failure before `TryMarkJoined` succeeds.

```text
JoinPlayer returned null
missing declaration
correlation mismatch
manager callback divergence
admission failure
```

Rollback must be idempotent and its result must remain visible in `LocalPlayerJoinResult`. A rollback failure does not replace the original failure; it upgrades the final result to `FailedRollback` and preserves both diagnostics.

### Unexpected external join

A joined callback without one compatible pending operation is rejected. The Player is not admitted, no Slot is marked Joined, and the bridge emits explicit evidence. Physical cleanup of that untracked Unity host is an explicit bridge responsibility in the implementation cut; it is never silently adopted.

### Deferred advanced request controls

The first implementation does not expose:

```text
requested PlayerSlot
requested playerIndex
requested splitScreenIndex
multiple-device pairing set
join cancellation
online/network identity
```

A later product need may add a separate advanced request shape. It must not expand the ordinary join request by default.
