...
Add it to the same GameObject as the gameplay `PlayerInput`, assign the `InputActionReference` for `Global/Pause`, and set the `Global`, `Player`, and `UI` action maps. Use **Apply/Rebuild Technical Binding** to create or validate the co-located `UnityPlayerInputGateAdapter`.
