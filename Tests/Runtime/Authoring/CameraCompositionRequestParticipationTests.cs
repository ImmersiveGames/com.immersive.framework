...
namespace Immersive.Framework.Authoring.Tests
{
    public sealed class CameraCompositionRequestParticipationTests
    {
        ...
        public void TwoCompositionsOnSameOutputUseNormalArbitration()
        {
            ...
            CameraPresentationRuntime second = fixture.CreateComposition("second", secondRig, 20);
        ...
        [Test]
        public void PresentationRequestUsesLifecycleOwnerAndLifetime()
        {
            using var fixture = new Fixture(CameraRigPresentationIntent.Fixed);
            CameraRequest request = fixture.Context.Winner;
            Assert.That(request.Owner.Kind, Is.EqualTo(CameraRequestOwnerKind.Session));
            Assert.That(request.Lifetime.Kind, Is.EqualTo(CameraRequestLifetimeKind.Session));
            ...
            Assert.That(request.Owner.OwnerScopeId.Value,
                Is.EqualTo(fixture.LifecycleContext.Owner.StableText));
        ...
        private sealed class Fixture : IDisposable
        {
                ...
                LifecycleContext =
                    new RuntimeScopeContext(
                        RuntimeContentOwner.Session(
                            $"camera-test-session-{suffix}",
                            $"camera-test-session-{suffix}"),
                        nameof(CameraCompositionRequestParticipationTests),
                        "presentation-test");

                Composition =
                    new CameraPresentationRuntime(
                        $"presentation-{suffix}");
            ...
            internal RuntimeScopeContext LifecycleContext { get; }
            internal CameraPresentationRuntime Composition { get; }
            ...
            internal CameraPresentationRuntime CreateComposition(
                string id, CameraRigComposer rig, int precedence)
            {
                ...
                var lifecycleContext =
                    new RuntimeScopeContext(
                        RuntimeContentOwner.Session(
                            $"camera-test-session-{_suffix}-{id}",
                            $"camera-test-session-{_suffix}-{id}"),
                        nameof(CameraCompositionRequestParticipationTests),
                        "presentation-test");
                var presentation =
                    new CameraPresentationRuntime(
                        $"presentation-{_suffix}-{id}");
                presentation.Configure(
                    ...
                    CameraPresentationTransitionMode.Blend,
                    lifecycleContext);
                presentation.AttachOutputSession(Output);
                presentation.AttachCameraSubjectAvailability(availability);
